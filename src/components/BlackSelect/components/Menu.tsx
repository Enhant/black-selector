...
const Menu = forwardRef<HTMLUListElement, IMenuProps>((props, ref) => {
    ...
    const getPositionStyle = useCallback((element) => { 
    ...
    const getItems = (children : React.ReactNode | undefined) => Children.map(children , (child, index: number) => {   
        if (index === activeItemIndex) {
            return React.cloneElement(child as React.ReactElement<any>, {
                ref: (instance: ((instance : HTMLUListElement | null) => void) | null | React.Ref<HTMLUListElement>  ) => {
    ...
    if (isOpen) {
        return createPortal(
            <Popover onClose={onClose} handleListKeyDown={handleListKeyDown}>
                <MenuList items={items} ref={ref} listClassName={listClassName} positionStyle={getPositionStyle(anchorRef)} onKeyDown={props.onKeyDown}/>
            </Popover>,
            containerEl,
        );
    }
